...
class Pipeline:
    ...
    # metodos publicos
    def get_palavras_sem_stops(self, text, custom_stops=None):
        stops = self.get_stop_words()
        if custom_stops is not None:
            stops = custom_stops
        sentencas = self.sent_tokenize(text)
        palavras  = self.tokenize(text.lower()) 
        
        return [plvr for plvr in palavras if plvr not in stops]
    ...
    def tokenize_tf_idf(self, texto):
        """
        quebra o texto em sentencas, quebra as sentencas em palavras, e reduz
        as palavras a seu radical
        """
    ...
    def _get_simple_ponto_sentenca_tf_idf(self, sentencas):
        ...
        os pre-tratamentos feitos no tokenize_tf_idf, devem ser feitos tambem as 
        stop words
        """
        ...
        tfidf = TfidfVectorizer(analyzer='word', tokenizer=self.tokenize_tf_idf, stop_words=stops, decode_error='ignore')
